...
ARQUIVOS_LOCAIS = {
    "adolescente": BASE_PATH / "adolescente-Calend.pdf",
    "adulto":      BASE_PATH /"adulto-Calend.pdf",
    "idoso":       BASE_PATH / "idoso-Calend.pdf",
    "prematuro":   BASE_PATH / "prematuro-Calend.pdf",
    "crianca":     BASE_PATH / "crianca-Calend.pdf",
}

ARQUIVOS_REMOTOS = {} # Caso precise baixar algo futuramente
ARQUIVO_SAIDA = BASE_PATH / "calendario_completo.json"
...
def processar():
    ...
    for categoria, caminho in ARQUIVOS_LOCAIS.items():
        if caminho.exists():
            print(f"Lendo: {caminho.name} ({categoria})")
            dados_extraidos = extrair_dados(str(caminho), categoria)
